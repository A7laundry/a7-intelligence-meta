...
def briefing():
    ...
    try:
        data = _get_service().generate_daily_briefing(days=days, platform=platform)
...
def recommendations():
    ...
    try:
        recs = _get_service().generate_recommendations(days=days, platform=platform)
        if severity:
...
def health():
    ...
    platform = request.args.get("platform")
    try:
        data = _get_service().build_account_health_snapshot(days=days, platform=platform)
    except Exception as e:
...
def refresh():
    ...
    try:
        svc = _get_service()
        briefing = svc.generate_daily_briefing(days=days, platform=platform)
        recs = svc.generate_recommendations(days=days, platform=platform)
        health = svc.build_account_health_snapshot(days=days, platform=platform)

        # Optionally persist top insights
        for r in recs[:5]:
            try:
                svc.save_insight(r)
            except Exception:
